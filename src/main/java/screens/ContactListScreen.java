...
public class ContactListScreen extends BaseScreen{
    ...
    public ContactListScreen ScrollingToLastContact(){
        Dimension dimensions = driver.manage().window().getSize();
        waitElement(addContactBtn, 5);
        MobileElement contact = contacts.get(7);
        Rectangle rect = contact.getRect();

        int start_y = (int) (dimensions.height * 0.2);
        int end_y = (int) (dimensions.height * 0.95);

        TouchAction<?> touchAction = new TouchAction<>(driver);
        touchAction.press(0, start_y)
                .moveTo(0, end_y)
                .release()
                .perform();
